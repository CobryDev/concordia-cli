...
class TestGenerateConcordiaConfig:
    ...
    def test_timestamp_type_mapping_details(self):
        ...
        timestamp_mapping = next(
            m for m in type_mapping if m['bq_type'] == 'TIMESTAMP')
    ...
    def test_string_type_mapping_details(self):
        ...
        string_mapping = next(
            m for m in type_mapping if m['bq_type'] == 'STRING')
    ...
    def test_integer_type_mapping_details(self):
        ...
        integer_mapping = next(
            m for m in type_mapping if m['bq_type'] == 'INTEGER')
...
class TestWriteYamlWithComments:
    ...
    def test_yaml_formatting_standards(self):
        ...
        connection_line = next(i for i, line in enumerate(
            lines) if 'connection:' in line)
